...
public class JigsawInteraction : MonoBehaviour
{
    ...
    public float JigsawMinDistance = 0.2f;
    ...
    public void IsJigsawUnselectedParts(GameObject partModel)
    {
        ...
        if (IsCUrrentUnselectedCombinate)
        {
            ...
            partModel.transform.DOMove(mappedGhostPiece.transform.position, 0.1f) // 移动到目标位置
                .SetEase(Ease.InOutSine) // 设置缓动类型
                .SetRelative(false); // 确保是绝对位置，而非相对位置

            partModel.transform.DORotate(mappedGhostPiece.transform.rotation.eulerAngles, 0.1f) // 旋转到目标旋转
                .SetEase(Ease.InOutSine)
                .SetRelative(false); // 确保旋转是绝对旋转

            Debug.LogWarning("执行自动吸附组合");
        }
        else
        {
